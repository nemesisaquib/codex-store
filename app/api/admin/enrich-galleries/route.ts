...
export async function GET() {
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const img5 = pool[(offset + 5) % pool.length];
      const img6 = pool[(offset + 6) % pool.length];
    ...
    return NextResponse.json({ ok: true, message: `Successfully updated ${updatedCount} products with 6 high-res gallery images each.` });
  } catch (e) {
