...
export async function GET() {
  try {
    ...
    const totalSubs     = ((await db.execute("SELECT COUNT(*) as c FROM newsletter")).rows[0] as {c:number}).c;
    const segments = {
      vip:      { count: (vipCustomers as unknown[]).length,  label: "VIP",           color: "#d4a017" },
      loyal:    { count: (await db.execute("SELECT COUNT(*) as c FROM customers WHERE tier='loyal'")).rows[0] as {c:number},    label: "Loyal",         color: "#3b82f6" },
      regular:  { count: (await db.execute("SELECT COUNT(*) as c FROM customers WHERE tier='regular'")).rows[0] as {c:number},  label: "Regular",       color: "#8b5cf6" },
