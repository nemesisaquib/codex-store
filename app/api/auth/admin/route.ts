...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = getDb();
    const admin = db.prepare("SELECT * FROM admin_users WHERE email=? AND password=?").get(email, password) as {id:string;name:string;email:string;role:string}|undefined;
...
export async function GET(req: NextRequest) {
  ...
  const db = getDb();
  const admin = db.prepare("SELECT id,name,email,role,last_login FROM admin_users WHERE id=?").get(session) as {id:string;name:string;email:string;role:string;last_login:string}|undefined;
